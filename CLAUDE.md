...
- Realized trading P&L uses WAVG cost basis matching on the **clean price** (accrued excluded — carry is booked by the accrual engine, never folded into trading realized), consistent with Bloomberg PRISM / Advent Geneva. `trading_gains.py` and `movements.py` both key off `price/100`, so they reconcile exactly.
- The daily P&L curve (`history.py`) is the true clean-price P&L (change in clean market value − clean cash invested) plus carry, so a round trip is counted **once**: `price_total` (SOD mark close→close + today's trades marked execution→close) has the realised slice carved out into `Realized`, giving `total = price_pnl + accrued + realized` with no double count between the daily mark and cost-basis realized.
