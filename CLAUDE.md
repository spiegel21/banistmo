...
- `src/bond_portfolio.py` — deterministic **CUSIP→portfolio** assignment so a bond's whole trade history sits in one book (the email parser's per-trade `portfolio` tag is advisory). `resolve_portfolios()` decides each bond's book by precedence: (1) **manual pin** — `data/bond_portfolio_map.csv` (`cusip,portfolio`), edited in the dashboard's **Data Editor → Portfolio Assignment** view; (2) **initial position** — a bond seeded in `initial_positions.csv` keeps its book; (3) **issuer inference** — a bond inherits its issuer's book when every initial-position bond from that issuer sits in ONE book. Issuer identity is the `issuer` field (preferred) else the 6-digit CUSIP prefix. When an issuer's initial bonds span more than one book the inference is ambiguous: those bonds are left **unassigned** (flagged in the editor for manual pinning) and keep whatever portfolio their trades already carry — never silently moved. `load_all_trades(assign_portfolios=True)` applies the resolved map to every trade's `portfolio` column at the load edge, so positions/P&L/filters all agree. Tables across the app are enriched with **Name** and the resolved **Portfolio** so each row is identifiable.
