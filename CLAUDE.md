...
- Total P&L = Realized + Price MTM + Accrued (three non-overlapping components). In `pnl_history` (`history.py`) the `Accrued` component is the true net interest carry: the daily mark-to-market change in accrued interest **plus the cash coupon** paid on any coupon date crossed that step. The accrued mark drops sharply on a coupon date (accrued resets to ~0) and the coupon cash lands on the same step, so the two net to the day's carry and a long never shows a spurious negative accrual across a coupon. Coupon cash is folded into `accrued` (not a separate column) and is booked on the holder-of-record SOD nominal independent of price availability; shorts pay it away. The current-MTM snapshot in `mtm.py` splits unrealized into `price_pnl` + `accrued_pnl` (the accrued *level*, always same-sign as the position) and does not carry realised coupons — that income lives only in the `pnl_history` accrued series.
